...
void Nav::defaultCommonSettings()
{
	m_cellSize = 0.3f;
	m_cellHeight = 0.2f;
	m_agentHeight = 2.0f;
...
void Nav::setCommonSettings(float cellSize,float cellHeight,
...
bool Nav::load(const char * path)
{
	printf("load:%s\n",path);
	return m_geom->load(m_ctx,path);
}
...
bool Nav::getRandomPos(float * pos)
{
	dtPolyRef ref;
	dtStatus status = m_navQuery->findRandomPoint(&m_filter,frand,&ref,pos);
	return dtStatusSucceed(status);
}

bool Nav::getHeight(const float * pos, float distance, float * height)
{
	float hitTime;
	float rayStart[3];
	rayStart[0] = pos[0];
	...
	if (hit) {
		*height = rayStart[1] + (rayEnd[1] - rayStart[1])*hitTime;
		return true;
	}
	return false;
}

bool Nav::getPath(const float * spos, const float * epos, float * path, int * npath)
{
	...
	*npath = 0;
	if (m_npolys)
	{
		...
		m_navQuery->findStraightPath(spos, nepos, m_polys, m_npolys,
			path, m_straightPathFlags,m_straightPathPolys, 
			npath, MAX_POLYS, m_straightPathOptions);
		return true;
	}
	return false;
}
